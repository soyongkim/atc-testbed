...
console.log = require('debug')('hidden');
var debug = require('debug')('viip:tas');
                    ...
                    if (sink_obj.con == 'hello') {
                        ...
                        for (j = 0; j < download_arr.length; j++) {
                            if (download_arr[j].ctname == sink_obj.ctname) {
                                ...
                                g_down_buf = sink_obj.con;
                                console.log('Item Description : ' + g_down_buf + ' <----');
                                debug('Item Description : ' + g_down_buf + ' <----');
...
function tas_watchdog() {
    if(tas_state == 'init') {
        ...
        upload_client.connect(useparentport, useparenthostname, function() {
            ...
            //debug('upload Connected');
            tas_download_count = 0;
            for (var i = 0; i < download_arr.length; i++) {
                console.log('download Connected - ' + download_arr[i].ctname + ' hello');
                //debug('download Connected - ' + download_arr[i].ctname + '- hello');
                debug('Connect Test - ' + download_arr[i].ctname + '- by hello message');
...
function saveLastestData(data) {
    ...
    if(g_sink_buf_start == 0) {
        ...
        if(count > 21){
            ...
            //여기서 실제로 읽어드린 데이터를 추출해서 해석하고 8. VR Service의 4-3 Item Description Request를 구현
            //var nValue = g_sink_ready[3] * 256 + g_sink_ready[4];
            var con = {};
            // VLC 프레임에서 받았다고 가정
            con.nValue = g_sink_ready[3];
            console.log('-- VLC Frame receive --');
            debug('DATA:' + g_sink_ready);
            if(g_sink_ready[3] == 1) {
                con.vtid = '/VT1';
            }
            else if(g_sink_ready[3] == 2) {
                con.vtid = '/VT2';
            }
            else if(g_sink_ready[3] == 3) {
                con.vtid = '/VT3';
            }
            else if(g_sink_ready[3] == 4) {
                con.vtid = '/VT4';
            }
            else if(g_sink_ready[3] == 5) {
                con.vtid = '/VT5';
            }
            else if(g_sink_ready[3] == 6) {
                con.vtid = '/VT6';
            }
            else {
                con.vtid = '/NonVT';
            }
            debug('-- VLC Frame receive --');
            if(tas_state == 'upload') {
                for(var i = 0; i < upload_arr.length; i++) {
                    if(upload_arr[i].ctname == 'CNT-VR-DESP') {
                        var cin = {};
                        cin.ctname = upload_arr[i].ctname;
                        cin.con = con;
                        //console.log('SEND : ' + JSON.stringify(cin) + ' ---->');
                        console.log(`SEND => [VT id]: ${con.vtid} [Item req]: ${con.nValue}`);
                        debug(`SEND => [VT id]: ${con.vtid} [Item req]: ${con.nValue}`);
                        upload_client.write(JSON.stringify(cin) + '<EOF>');
                        break;
                    }
                }
