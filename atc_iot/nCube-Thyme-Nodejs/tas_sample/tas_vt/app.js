...
var debug = require('debug')('viip:tas_vt');
                    ...
                    if (sink_obj.con == 'hello') {
                        ...
                        for (j = 0; j < download_arr.length; j++) {
                            if (download_arr[j].ctname == sink_obj.ctname) {
                                g_down_buf = JSON.stringify({id: download_arr[i].ctname, con: sink_obj.con});
                                console.log(g_down_buf + ' <----');
                                if(sink_obj.ctname == 'CNT-VLC-CONTROL') {
                                    debug(`VT state change to 'ON' <----`)
                                    control_light(sink_obj.con.state);
                                    //vt_state = JSON.stringify(sink_obj.con);
                                }
                                else if(sink_obj.ctname == 'CNT-VLC-DATA') {
                                    debug(`VLC Frame ready <----`);
                                    debug(`[Service id] : ${sink_obj.con.serviceid}`);
                                    debug(`[GW id]      : ${sink_obj.con.gwid}`);
                                    debug(`[VT id]      : ${sink_obj.con.vtid}`);
                                    debug(`[ups id]     : ${sink_obj.con.upsid}`);
                                    debug(`[ups pwd]    : ${sink_obj.con.upspwd}`);
                                }
                                break;
                            }
                        }
...
function control_light(comm){
    console.log('comm : ' + comm);
    //debug(`TEST: ${comm}`);
    var cmd = 'sudo ./vlc ' + comm;
